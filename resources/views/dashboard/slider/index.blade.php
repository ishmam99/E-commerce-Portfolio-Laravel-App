          ...
          <tbody>
            @foreach($sliders as $slider)
              <tr>
               ...
               <td><img src="{{setImage($slider->photo)}}" height="150px" alt=""></td>
                <td>{{ $slider->name }}</td>
                <td>{{ $slider->status==0 ? 'Published' : 'Unpublished' }}</td>
                <td nowrap="nowrap">
                  <a href="{{ route('sliders.edit', $slider->id) }}" class="btn btn-icon btn-light btn-hover-primary btn-sm mx-3">
                   ...
                   <a href="{{ route('sliders.show', $slider->id) }}" class="btn btn-icon btn-light btn-hover-primary btn-sm mx-3">
                    <span class="svg-icon svg-icon-md svg-icon-primary">
                      <img src="https://img.icons8.com/office/16/000000/details-pane.png"/>
                    </span>
                  </a>
                  <form method="post" action="{{ route('sliders.destroy', $slider->id) }}" class="d-inline-block">
                    @csrf
                    @method('DELETE')
